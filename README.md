...
Open from `Tools > iPhysicsHub` — two windows: **Camera Updater** and **2D-3D Toggle**.
...
1. Open **Camera Updater** (`Tools > iPhysicsHub > Camera Updater`).
2. In the install section, pick the controller template and click **Install / Update in Project**
   (default `Assets/Scripts`). It compiles in.
...
Open **2D-3D Toggle** (`Tools > iPhysicsHub > 2D-3D Toggle`):

1. Pick the **Controller type** (dropdown defaults to the project's latest controller). The toggle
   script is **retargeted to that controller on install** (namespace-qualified), so it follows a
   controller upgrade instead of staying bound to the old version.
2. **Install toggle files** — copies the toggle script + icons in (set destinations as needed).
3. After it compiles, **Add toggle to scene** — builds the Canvas/button/icon hierarchy and
   wires it (finds the scene controller; falls back to auto-find at Play). Re-run install with the
   new controller selected after upgrading to retarget the toggle.

## Maintainer

In **Camera Updater**, unlock the gated **Maintainer** area, assign a tested `.cs` and **Save to
Package** (optionally under a new class name) to write it into `Templates~/`. Then bump
`package.json` version, commit, `git push`. Capture only works on the embedded/source package, not
a read-only git cache.
